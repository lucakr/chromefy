...
#4 - [Optional] ChromeOS image from actual device being used (needed to fetch override files to improve compatibility)
#n - [Optional] List of files/folder to copy from the actual device's ChromeOS image to the final image (eg /etc/gesture)
...
if [ ! -d /home/chronos/actual ]; then mkdir /home/chronos/actual; fi
...
if [ ! -f "$4" ]; then echo "Image $3 not found"; abort_chromefy; fi
chromeos_actual=`losetup --show -fP "$4"`
mount "$chromeos_actual"p3 /home/chronos/actual -o loop,ro 2>/dev/null
if [ ! $? -eq 0 ]; then echo "Image $3 does not have a system partition (corrupted?)"; abort_chromefy; fi
